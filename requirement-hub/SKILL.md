...
description: Generates a single-page requirement traceability hub (requirement.html) — with an auto-derived interactive node graph — plus clickable, commentable UI mockups (make-front/SCR-*.html) and PlantUML diagrams (diagrams/*.puml) from raw requirement sources (PDF/DOCX/PNG), or reverse-engineered as-built from an existing app. Use in a "*-requirement" repo, or when the user wants to build or update a requirement.html, convert requirements into use cases, produce a traceability doc linking Workflows to Use Cases to Screens to APIs to Test Cases with unique IDs, add or improve the interactive requirement graph / node map, scaffold reviewable UI mockups, or add PlantUML diagrams (context / use-case / ERD / state / sequence). Trigger phrases include "requirement.html", "use case document", "traceability", "graph node", "node graph", "แผนผังกราฟ", "make-front", "UI mockup", "plantuml", "diagram", "as-built", "ศูนย์รวม requirement", or "สร้าง requirement".
---
