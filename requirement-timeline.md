...
**สถานะ: UI เสร็จเป็นส่วนใหญ่ เหลือ logic หลังบ้าน + ของใหม่จากประชุม** — แตกเป็นการ์ดใน `requirement.html`:

**✅ ทำแล้ว / ไม่ต้องทำ (ยืนยันจากโค้ด):**
- **ค้นหานักเรียน** (ไม่ต้องพิมพ์ "น้อง" + ค้นด้วยเบอร์) — `searchStudents` ilike substring ชื่อ/ชื่อเล่น/เบอร์ **ทำแล้ว** (UC-001)
- **เช็คอิน QR แทนพิกัด** — ระบบใช้ QR/token อยู่แล้ว **ไม่มีโค้ด GPS** เลย (UC-009); ความกังวลเรื่องพิกัดตกไป

**🟡 Partial / ต้องแก้เพิ่ม:**
- **UC-032 LINE bot 2 ภาษา** — `sick/leave/checkin` รับ EN แล้ว แต่รูปแบบมีเลข (`ลา 1`) + `qr`/`เพิ่มนักเรียน` + ข้อความตอบกลับ ยังไทยล้วน
- **UC-016 แถบสีรายได้ครูบนปฏิทิน** — เปลี่ยนจาก "ซ่อนครูเมื่อเกินเพดาน" → "โชว์แถบสี เขียว/เหลือง/แดง + ยังจองได้"; แถบสีมีแล้วในหน้าจัดการครู แต่ปฏิทินยังซ่อน (`bookable = active && !overLimit`)
- **UC-003 Auto-Move** — "ย้ายวัน→ตัดเก่า+ต่อท้าย นับครบ" ปัจจุบันทำผ่าน **แจ้งลา (UC-010)** เท่านั้น · **ต้อง clarify** ว่าอยากให้เลือกวันใหม่เอง+นับครบ (ยังไม่มี) ไหม

**🔴 ยังไม่เริ่ม (ของใหม่):**
- **UC-027** ยืนยันคาบแบบกลุ่ม (Bulk Confirm)
- **UC-028** LINE แจ้งผู้ปกครองเมื่อสมัครคอร์สสำเร็จ (ครู/วันเริ่ม/จำนวน ชม.) — `createCoursePackage` ยังไม่ push
- **UC-029** กฎแจ้งลาล่วงหน้าตามประเภทครู (FT/PT ≥ 1 ชม., FL ≥ 2 ชม.)
- **UC-030** เวลาทำงานครูระดับช่วงเวลา (เช้า/บ่าย) — ปัจจุบัน `work_days` ราย "วัน" เท่านั้น
- **UC-031** แก้/ขยายคอร์สด้วยมือ (ลาล่วงหน้าตั้งแต่สมัคร + ขยาย expiry)
- **UC-033** แยกวิชา Bike / Scooter (ตอนนี้รวมเป็น "Bike / Scooter / Balance Cruiser" ใน seed)

**🔮 การบ้าน/อนาคต (WF-011, ยังเป็นงานออกแบบ):**
- **UC-034** Multi-branch (schema รองรับหลายสาขา, ครูข้ามสาขา, admin ดูรวม)
- **UC-035** Sync ตารางสอนเข้า Google Calendar / ปฏิทินมือถือครู
- **Option C สต๊อก/POS + Dashboard ยอดขายรายวัน** = UC-022/023/024 (Partial: API มี, backoffice-front ยังไม่เริ่ม)

**หมายเหตุขัดแย้ง:** ข้อ Auto-Move (1.3) ทับกับการถอด flow "ย้ายของเดิม+รอผู้ปกครอง" (UC-005/006) ที่เพิ่งเอาออก — ต้องเคาะพฤติกรรมกับเจ้าของก่อนลงมือ

---

## 2026-07-11 — Voucher ใช้งานจริงได้ (จองด้วยวอยเชอร์ + ตัดชั่วโมง)
> ที่มา: เจ้าของงาน — "UC-008 ออก voucher ได้ แต่ยังใช้งานจริงไม่ได้"

**ปัญหาเดิม:** ออกวอยเชอร์ได้ แต่ไม่มีทาง "ใช้" — โมดอลจองมีประเภท Voucher แต่ไม่ส่ง `voucherId`
(คาบไม่ผูกวอยเชอร์ → ไม่ตัดชั่วโมง/ไม่เช็คอายุ) และไม่มี `GET /api/vouchers` (แท็บวอยเชอร์เป็น mock)

- **จองด้วยวอยเชอร์:** โมดอลจอง (SCR-003) เมื่อเลือกประเภท **Voucher** จะโหลดวอยเชอร์ของนักเรียนคนนั้น
  และให้เลือก (เฉพาะที่ **ชั่วโมงเหลือ + ไม่หมดอายุ**) → ส่ง `voucherId` เข้ากับ `POST /api/bookings`.
  มาเรียน (attend) → ตัด `usedHours` +1 ตามเดิม (B.5).
- **บังคับความถูกต้องที่ backend:** จองแบบ Voucher **ต้องมี** `voucherId` (zod refine) และวอยเชอร์
  ต้องเป็น**ของนักเรียนที่เลือก** (ownership check) มิฉะนั้น 400.
- **`GET /api/vouchers?studentId&q`** (API-009): รายการวอยเชอร์จริง — แท็บวอยเชอร์ (SCR-005) เลิกใช้ mock
  และเป็นแหล่งข้อมูลตัวเลือกตอนจอง.
